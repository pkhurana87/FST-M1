...
public class Chrome_Activity1 {
	...
	public void ToDoList() throws InterruptedException {
		...
		// driver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true)).scrollIntoView(text(\"To-Do List Elements get added at run time\"))");
		// driver.find
		...
		for(String in : input) {
		//driver.findElementById("taskInput").sendKeys(in);
		...
		for(String strike : input) {
//		int act_size = driver.findElements(MobileBy.AndroidUIAutomator("text(\""+strike+"\")")).size();
//		int exp_size = 0;
